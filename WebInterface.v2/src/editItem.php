...
if ($mysqli->connect_errno) {
    ...
    if(isset($_GET['id'])){
        ...
        //Prepare a statement
        if(!($stmt = $mysqli->prepare("SELECT name, cat_id FROM contents WHERE id=?"))){
        ...
        $name;
        $cat_id;
        if(!($stmt->bind_result($name, $cat_id))){
...
?>
        ...
        <div id="businesses" class="marker"></div>
          ...
          <form class="form-horizontal" id="editCategory">
            <div class="form-group">
              ...
              <div class="col-sm-10">
                <input type="text" class="form-control" id="item-name" placeholder="Item Name" value=<?php echo "\"" . $name . "\"" ?> required>
              </div>
            </div>

            <div class="form-group">
              <label for="item-cat" class="col-sm-2 control-label">Category</label>
              <div class="col-sm-10">
    ...
    <?php
                ...
                if(!($stmt->bind_result($listed_cat_id, $cat_name))){
                    echo "<p>Binding output parameters failed: (" . $stmt->errno . ") " . $stmt->error;
                }

                //Fetch results        
                while($stmt->fetch()){ 
                    if($cat_id == $listed_cat_id){          
    ?>    
                        <option value=<?php echo "\"" . $listed_cat_id . "\""?> selected> <?php echo $cat_name?>
    <?php
                    }
                    else{
    ?>    
                        <option value=<?php echo "\"" . $listed_cat_id . "\""?>> <?php echo $cat_name?>
    <?php
                    }
                }
    ...
    ?>
            ...
            <div class="form-group col-sm-3"  style="margin-left: 40px;">
              <div class="col-sm-offset-6 col-sm-6">
                <input type="button" id="edit-item" class="btn btn-success" value="Edit Item" style="width: 113px;" onclick=<?php echo "\"editItem(" . $item_id . ")\""?>>
              </div>
            </div>
            <div class="form-group col-sm-2">
              <div class="col-sm-12">
                <a href="home.php#items" class="btn btn-default" style="width: 113px;">Back</a>
              </div>
            </div>
